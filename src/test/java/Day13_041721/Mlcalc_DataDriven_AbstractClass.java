...
public class Mlcalc_DataDriven_AbstractClass extends Abstract_Class {
    @Test
    public void Monthly_payment_mlcalc() throws InterruptedException, WriteException, IOException, BiffException {
        //Step1: locate the path for readable file that we created in excel
        Workbook readableBook = Workbook.getWorkbook(new File("src/main/resources/DataDrivenTesting.xls"));

//Step 2: create a writable workbook that will mimic the data from readable
        WritableWorkbook writableWorkbook = Workbook.createWorkbook(new File("src/main/resources/Mlcacl_DataDriven_Result.xls"),readableBook);
